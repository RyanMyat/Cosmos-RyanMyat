...
def main():
   ...
   while not done:
      ...
      for event in pygame.event.get():
         ...
         # User Presses Down On Mouse
            ...
            if game_over == False:
               ...
               lasers_rot.append(pygame.transform.rotate(laser, 270-(math.atan2(cursor[1]-(player_pos[1]+30), cursor[0]-(player_pos[0]+30)))*(180/pi)))
      ...
      # Asteroids Timer 1
      if ast1_timer == 0:
         asteroids1.append([random.randint(0,800), 630, random.randrange(-1,2,2), random.randint(-4,0)])

         # Reset Timer 1
         ast1_timer = 300-(ast1_num*2)
         if ast1_num >= 125:
            ast1_num = 125
         else:
            ast1_num += 1
      ...
      # Asteroids Timer 2
      if ast2_timer == 0:
         asteroids2.append([800, random.randint(0,600), random.randint(-6,-2), random.randrange(-1,2,2)])

         # Reset Timer 2
         ast2_timer = 325-(ast2_num*3)
         if ast2_num >= 100:
            ast2_num = 100
         else:
            ast2_num += 1
      ...
      for i in range(len(asteroids3)):
         ...
         for j in range(len(lasers)):
            ...
            if asteroid3rect.colliderect(laserrect):
               ...
               del lasers[j]
               del lasers_rot[j]
               explode.append([laserrect.left, laserrect.top, random.randint(-4,0), random.randrange(-1,2,2)])
               break
               doublebreak3 = True

            if doublebreak3 == True:
               break
      ...
      font = pygame.font.SysFont('kenvector_future.ttf', 20, True, False)
      text = font.render(" " + str(life), True, WHITE)
      screen.blit(text, [75, 43])
      ...
      # Game Over
      if game_over == True:
         font = pygame.font.SysFont('kenvector_future.ttf', 50, True, False)
         text = font.render("G A M E  O V E R", True, WHITE)
         text_rect = text.get_rect()
