...
def main():
   ...
   asteroid1_rect = asteroid1.get_rect()
   ...
   while not done:
      ...
      # Asteroids Timer
      if ast_timer == 0:
         asteroids.append([random.randrange(0,360,50), random.randint(0,800), -30, random.randrange(-1,2,2), random.randint(1,3)])

         # Reset Timer
         ast_timer = 300-(ast_num*2)
         if ast_num >= 30:
            ast_num = 30
         else:
            ast_num += 1
      ...
      for i in range(len(asteroids)):
         ...
         asteroids[i][2] += asteroids[i][4]

         asteroids_rot.append(pygame.transform.rotate(asteroid1, asteroids[i][0]))
